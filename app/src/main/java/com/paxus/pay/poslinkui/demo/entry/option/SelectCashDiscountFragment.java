...
public class SelectCashDiscountFragment extends AOptionEntryFragment {
    ...
    @Override
    protected List<SelectOptionsView.Option> generateOptions(String[] tempOptions) {
        List<SelectOptionsView.Option> options = new ArrayList<>();
        for(int i=0; i<tempOptions.length; i++){
            try {
                JSONObject jsonObject = new JSONObject(tempOptions[i]);
                String title = jsonObject.getString(EntryExtraData.PARAM_TITLE);
                long amount = Long.parseLong(jsonObject.optString(EntryExtraData.PARAM_TOTAL_AMOUNT, "0"));
